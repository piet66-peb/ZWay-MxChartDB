/**
 * Calculates today's sunrise and sunset hours in local time (or in the given tz) 
 * for the given latitude, longitude.
 *   The tz parameter is mainly for the possible circumstance that your system 
 *   timezone does not match the location
 ...
 * @param {Integer} tz Timezone hour offset. e.g. Pacific/Los Angeles Standard 
 * Time is -8 (Optional, defaults to system timezone)
 * @return {Array} Returns an array of length 2 with the sunrise and sunset times 
 * as floats on 24-hour time.
 *                    e.g. 6.5 is 6:30am, 23.2 is 11:12pm, 0.3 is 12:18am
 *                 Returns an array with [null, -1] if the sun never rises, 
 *                 and [-1, null] if the sun never sets.
 *
 * ### LICENSING ###
 * 
...
function suntimes(day, lat, lng, tz) {
    'use strict';
    var d;
    if (day) { 
        d = new Date(day);
    ...
    var C = 1.9148 * Math.sin(M * radians) + 0.02 * Math.sin(2 * M * radians) + 
        0.0003 * Math.sin(3 * M * radians);
    var L = (M + 102.9372 + C + 180) % 360;
    var j_transit = solar_noon + 0.0053 * Math.sin(M * radians) - 
    ...
    //convert julian time (hour.frctal) > utc
    function toUTC(dat, jul) {
        var h = Math.floor(jul);
        var m = Math.floor((jul-h)/1*60);
        var m_c = h + m/60*1;
        var s = Math.floor((jul-m_c)/1*60*60);
        d.setHours(h,m,s);
        return d.getTime();
    }
    var ris = toUTC(d, local_rise);
